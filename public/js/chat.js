$(function () {
    ...
    let log = (message, options=undefined) => {
    ...
    let addChatMessage = (data, options) => {
        ...
        let $messageBodyDiv = $('<span class="messageBody">')
            .text(data.message)
        let $messageDiv = $('<li class="message" title="' + new Date(data.hora).toLocaleString() + '"/>')
    ...
    $window.keydown(event => {
        ...
        if (event.which === 13) {
            ...
            socket.emit('events', {message: $inputMessage.val(), user: 'tuba'})
            if (username) {
