...
let putOnQueue = (data) => {
    ...
    let $firstChildren = $("#selected").children().first()
...
$(document).ready(() => {
    ...
    socket.on('addMusicProgress', (data) => putOnQueue(data) )
    ...
    $('.label').on('click', () => document.getElementById("upio_input").click())
    $('.checkbox').click((e) => $(e.target).toggleClass('checked'))
    ...
    $("#play").on('click', (e) => {
        if (e.target.className.includes('playing')) dom.pause()
        else dom.play()
    });
    dom.onvolumechange = () => {
    ...
    dom.ontimeupdate = () => {
        ...
        let buf;
        try {
            buf = dom.buffered.end(dom.buffered.length - 1) / coef;
        }catch (err) {
            buf = 0;
        }
    ...
    if(current) {
        ...
        $('#np').text(current)
        let nextUrl =  '/' + user + '/' + next
        $('body').append($('<link>').attr('rel', 'prefetch').attr('href', nextUrl))
    }
})
